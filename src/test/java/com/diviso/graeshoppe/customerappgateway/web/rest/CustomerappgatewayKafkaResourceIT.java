...
public class CustomerappgatewayKafkaResourceIT {
    ...
    @Autowired
    private CustomerappgatewayKafkaProducer producer;

    @Autowired
    private CustomerappgatewayKafkaConsumer consumer;
    ...
    @BeforeEach
    public void setup() {
        CustomerappgatewayKafkaResource kafkaResource = new CustomerappgatewayKafkaResource(producer);
    ...
    public void producedMessageHasBeenConsumed() throws Exception {
        restMockMvc.perform(post("/api/customerappgateway-kafka/publish?message=test"))
